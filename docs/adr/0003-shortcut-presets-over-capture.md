...
- Adding a Preset means touching a fixed list in `src/shared/shortcut-options.ts`, `src/shared/types.ts`, and `src/bun/utils/keyboard/keyboard-events.ts`. A Preset with a Trigger Key also needs `windowsUsesModifierReleaseHold` in `shortcut-options.ts` plus a matching case in `isWindowsModifierReleaseEvent` in `src/bun/setup-recording.ts`, or its hold never ends on Windows. That is five files for one Preset. The count of Presets is now 12; if it grows much past a dozen, revisit capture.
