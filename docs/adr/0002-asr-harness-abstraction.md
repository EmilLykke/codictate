...
- ~~**The translate Speech Model swap is a Speech Model concern, not a Harness concern.** `resolveTranslateModelId()` in `src/shared/whisper-models.ts` is untouched by this decision: when Translate to English is requested, Codictate swaps to a translate-capable Speech Model (Small or Large), because turbo cannot translate at all. That swap is what makes Translate to English work.~~ **Superseded by `docs/adr/0005-no-runtime-fallbacks-for-dictation.md`.** There is no swap: Translate to English is unavailable while a Speech Model that cannot translate is selected, and `src/shared/whisper-models.ts` is deleted. The claim that this was never a Harness concern still holds — it is the swap that is gone, not its classification.
- Backend and language follow the Speech Model that runs. An hviske run passes `--backend cohere` and pins Danish. ~~A Translate to English request on an hviske selection resolves away to a translate-capable Whisper Speech Model, which inherits neither the backend nor the pin.~~ **Superseded by `docs/adr/0005-no-runtime-fallbacks-for-dictation.md`** — the Speech Model that runs is now always the one the user selected, so the distinction this bullet drew has no remaining case. None of it was a Harness concern; it is the same binary either way.
